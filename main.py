...
app.config['MAIL_PASSWORD'] = "ynkkfkhomwppejkx"  # not your actual Gmail password!
...
def contact_email():
    ...
    try:
        # Prepare the email
        msg = EmailMessage()
        msg['Subject'] = f"QuickMart Contact - {subject}"
        msg['From'] = email
        msg['To'] = 'futsumhal@gmail.com'  # Replace this with your real email
        msg.set_content(f"From: {name} <{email}>\n\n{message}")

        # Send the email (this is using Gmail SMTP for example)
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp:
            smtp.login('futsumhal@gmail.com', 'ynkkfkhomwppejkx')  # Use app password, not Gmail password
